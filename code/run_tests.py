import unittest
from tests.testDynamics import TestDynamics
from tests.testInputOutput import TestInputOutput
from tests.testAlgorithm import TestAlgorithms
from tests.testUser import TestUser
from tests.testPopulation import TestPopulation
from tests.testSaveLoad import TestSaveLoad
from tests.testRun import TestRun
from tests.testSamplers import TestSamplers

unittest.main(verbosity=2)
